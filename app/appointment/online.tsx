...
import { availableSlots, consultationReasons } from '@/mocks/appointments';
...
export default function OnlineAppointmentScreen() {
  ...
  useEffect(() => {
    ...
    // If service is specified in params, set it as reason
    if (params.service) {
      const reasonId = params.service.toString();
      const reason = consultationReasons.find(r => r.id === reasonId);
      if (reason) {
        setSelectedReason(reason.name);
      }
    }
    
    // Set home visit to false for online appointments
